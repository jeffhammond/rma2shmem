...
int MPI_Init_thread(int *argc, char ***argv, int required, int *provided)
{
    ...
    int mpi_required, mpi_provided;

    int rc = PMPI_Init_thread(argc, argv, mpi_required, &mpi_provided);
    if (rc != MPI_SUCCESS) return rc;
    ...
    if ((shmem_provided >= SHMEM_THREAD_MULTIPLE) && (mpi_provided >= MPI_THREAD_MULTIPLE)) {
        *provided = MPI_THREAD_MULTIPLE;
    } else if ((shmem_provided >= SHMEM_THREAD_SERIALIZED) && (mpi_provided >= MPI_THREAD_SERIALIZED)) {
        *provided = MPI_THREAD_SERIALIZED;
    } else if ((shmem_provided >= SHMEM_THREAD_FUNNELED) && (mpi_provided >= MPI_THREAD_FUNNELED)) {
